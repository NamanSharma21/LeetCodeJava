...
public class InvertBinaryTree {
    public static void main(String[] args) {
        ...
        System.out.println("InvertBinaryTree : " + invertBinaryTree.invertTreeDFSPreOrder(root));
    }
 ...
 * Given the root of a binary tree, invert the tree (mirror it about its
 * vertical axis) and return the root of the inverted tree.
 *
 * Inverting means: for every node in the tree, swap its left and right
 * children — recursively, all the way down.
 *
 * Input Format:
 ...
 * At every single node in the tree, swap its left and right child
 * pointers. This must happen at every level, not just the root.
 *
 *
 * 2. INTUITION
 * -------------
 * Core Idea (Think Like a Human):
 * Imagine holding a mirror up to the right side of the tree. Every
 * node's left child becomes its right child, and vice versa.
 *
 * How would you reason through it?
 * 1. Stand at the root. Swap its left and right children.
 * 2. Now go to the left subtree (which was originally the right) —
 *    do the same.
 ...
 *
 * This is naturally recursive — the definition of "invert this
 * subtree" is the same at every node.
 *
 * What Makes It Interesting?
 * - Classic example of recursive tree thinking
 * - Can also be solved iteratively using BFS or DFS — good for
 *   understanding traversal
 * - Looks simple but tests whether you truly understand tree
 *   structure and pointer manipulation
 * - Famous for being tweeted about by Max Howell (creator of Homebrew)
 *   who was rejected from Google for not solving it on the whiteboard!
 *
 *
 * 3. APPROACH OVERVIEW
 * ---------------------
 * #  | Approach                    | Key Idea
 * ---|-----------------------------|----------------------------------
 * 1  | Recursive DFS (Post-order)  | Recurse to leaves first, swap on
 *    |                             | the way back up
 * 2  | Recursive DFS (Pre-order)   | Swap first, then recurse into
 *    |                             | swapped children
 * 3  | Iterative BFS (Queue)       | Level-order traversal, swap
 *    |                             | children at each node
 * 4  | Iterative DFS (Stack)       | Explicit stack mimics recursion
 *
 * Use When:
 * 1 - Always — cleanest solution
 * 2 - Alternative recursive style
 * 3 - When stack overflow is a concern (very deep trees)
 * 4 - When you want iterative DFS specifically
 *
 * Optimal?  All four are O(n) time; #1/#2 are recommended for
 * interviews, #3 is safest for very deep trees.
 *
 * Recommended: Recursive Post-order or Pre-order — both are clean,
 * intuitive, and interview-ready. BFS is great to mention as an
 * alternative.
 *
 *
 ...
 * - We visit every node exactly once (there's no way to skip any node —
 *   each one needs its children swapped)
 ...
 * - Best case (balanced tree): h = log2(n) -> O(log n)
 ...
 *  2    | [7, 6, 9]          | 2 (now left of 4) | 2.left=3, 2.right=1
 ...
 *
 * Note: After processing node 4, its children become [7, 2] in the
 * queue. Each gets its children swapped when dequeued.
 *
 *
 * 7. EDGE CASES
 * ---------------
 ...
 * -------------------------------|---------------------|----------------------|----------------------------------------
 * Empty tree                     | root = null         | null                 | Base case `if (root == null) return null` handles this
 ...
 * Skewed tree (like linked list) | 1->2->3->4->5       | Reversed             | Recursive: O(n) stack depth — risk of
 *                                 |                     |                      | StackOverflow for very large n.
 *                                 |                     |                      | BFS/Stack iterative: Safe
 * Duplicate values                | [2, 2, 2]           | [2, 2, 2]            | Values don't matter — only structure changes
 * Negative values                 | [-1, -2, -3]        | [-1, -3, -2]         | Values are irrelevant to the algorithm
 *
 * IMPORTANT WARNING:
 * For the recursive approach on deeply skewed trees (essentially a
 * linked list of 10,000+ nodes), Java's default stack size will cause
 * a StackOverflowError. The iterative BFS or DFS approach avoids this
 * entirely since it uses a heap-allocated Queue/Stack instead of the
 * call stack.
 *
 *
 * 8. FINAL SUMMARY
 ...
 * ----------------------|------------------|--------------------|----------------|------------------
 * Recursive Post-order  | Cleanest         | O(log n) balanced  | Risk on skewed | Highest
 * Recursive Pre-order   | Clean            | O(log n) balanced  | Risk on skewed | High
 * Iterative BFS         | Moderate         | O(n) worst         | Always safe    | High
 * Iterative DFS (Stack) | Moderate         | O(h)               | Always safe    | Medium
 *
 * Recommendation:
 * Use Recursive Post-order in interviews — it's the most elegant and
 * demonstrates deep understanding of recursion. Mention Iterative BFS
 * as a follow-up to show awareness of stack overflow risks in
 * production systems.
 *
 * What to Remember:
 * Pattern: This is a pure recursive tree structural transformation
 * problem. The key insight is that inverting a tree = swap children +
 * recursively invert both subtrees. Any tree traversal (pre, post,
 * BFS, DFS) works as long as you swap at every node. Think: "What do
 * I need to do at each node?" — and the answer is always just swap.
 *
 *
 ...
 * - One of the Top 10 most famous LeetCode problems due to the viral
 *   tweet — almost every FAANG interviewer knows it, making it a
 *   must-know for any coding interview
 */
// @formatter:on
