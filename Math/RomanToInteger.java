...
public class RomanToInteger {
    ...
    public int romanToInt(String s) {
        ...
        for (int i = 0; i < numeralCharArray.length; i++) {
            System.out.println("C : " + numeralCharArray[i] + " N: " + number + " L : " + lastNumeral);
            int numeral = numeralMap.get("" + numeralCharArray[i]);
            if (numeral > lastNumeral) {
                int remainder = numeral - lastNumeral;
                number -= lastNumeral;
                number += remainder;
            } else {
                number += numeral;
